...
class Builder FLATBUFFERS_FINAL_CLASS {
 public:
  ...
  template<typename F> size_t Vector(const std::string& key, F f) {
    auto start = StartVector(key.c_str());
    f();
    return EndVector(start, false, false);
  }
  ...
  template<typename F> size_t Map(const std::string& key, F f) {
    auto start = StartMap(key.c_str());
    f();
    return EndMap(start);
  }
  ...
  template<typename T> void Map(const std::unordered_map<std::string, T> &map) {
    auto start = StartMap();
    for (auto it = map.begin(); it != map.end(); ++it)
        Add(it->first.c_str(), it->second);
    EndMap(start);
  ...
  template<typename T> void Add(const std::string& key, const T &t) {
    Key(key);
    Add(t);
  }
  ...
  template<typename T> void Add(const std::unordered_map<std::string, T> &map) {
    Map(map);
  }
